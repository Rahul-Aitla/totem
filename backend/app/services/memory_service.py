...
class MemoryService:
    ...
    def _fetch_recent_memories(self, db: Session, user_session_id: str, limit: int = 5):
        session_memories = (
            db.query(MemoryNode)
            .filter(
        ...
        for memory in session_memories + cross_session_memories:
    ...
    def extract_and_store_memory(self, db: Session, user_session_id: str, text: str):
        ...
        """
        # First, get recent memories across sessions so we can keep continuity between sessions.
        existing_memories = self._fetch_recent_memories(db, user_session_id, limit=10)

        existing_context = ""
        if existing_memories:
            existing_context = "Existing memories for this user:\n" + "\n".join(
                [f"- ID: {m.id}, Fact: {m.fact_text}" for m in existing_memories]
            )
            ...
            for fact in facts:
                if fact.get("action") == "update" and fact.get("existing_id"):
                    ...
                    try:
                        ...
                        node = (
                            db.query(MemoryNode)
                            .filter(MemoryNode.id == target_id)
                            .first()
                        )
                        if node:
                        ...
                        print(
                            f"Skipping invalid UUID from Gemini: {fact['existing_id']}"
                        )
                        continue
    ...
    def get_relevant_context(self, db: Session, user_session_id: str, limit: int = 5):
        """
        Retrieve relevant context for the current session and recent history
        across previous sessions for continuity.
        """
        memories = self._fetch_recent_memories(db, user_session_id, limit=limit)

        context_lines = []
        for memory in memories:
            prefix = (
                "current session"
                if memory.user_session_id == user_session_id
                else f"previous session {memory.user_session_id}"
            )
            context_lines.append(f"- ({prefix}) {memory.fact_text}")
    ...
    def merge_memories(
        self, db: Session, primary_id: uuid.UUID, secondary_id: uuid.UUID
    ):
